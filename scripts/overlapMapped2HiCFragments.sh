...
do
    ...
    preBAM=`basename ${r} | sed -e 's/.sam$/_interaction/'`
    ...
    if [ -f ${datadir}/${outSAM} ]
    then 
	cmd="${SAMTOOLS_PATH}/samtools view -bS ${datadir}/${outSAM} | ${SAMTOOLS_PATH}/samtools sort - ${datadir}/${preBAM}"
	exec_cmd $cmd
	cmd="${SAMTOOLS_PATH}/samtools index ${datadir}/${preBAM}.bam"
	exec_cmd $cmd
    fi
done
echo "## done !"
