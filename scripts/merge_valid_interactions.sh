...
do
	...
	then
	    ...
	    cmd="cat ${IN_DIR}/${RES_FILE_NAME}/*.validPairs > ${DATA_DIR}/${RES_FILE_NAME}/${RES_FILE_NAME}_allValidPairs"
...
awk -F\"\t\" 'BEGIN{c1=0;c2=0;s1=0;s2=0}(c1!=\$2 || c2!=\$5 || s1!=\$3 || s2!=\$6){print;c1=\$2;c2=\$5;s1=\$3;s2=\$6}' > ${DATA_DIR}/${RES_FILE_NAME}/${RES_FILE_NAME}_allValidPairs"
	    exec_cmd $cmd >> ${logfile} 2>&1
	fi
	...
	echo -e $RES_FILE_NAME"\t"$nbdup >> ${logfile}
       ...
       ## On Target                                                                                                                                                      
	    ...
	    cmd="${PYTHON_PATH}/python ${SCRIPTS}/onTarget.py -i ${DATA_DIR}/${RES_FILE_NAME}/${RES_FILE_NAME}_allValidPairs -t ${CAPTURE_TARGET} -p -v 1> ${DATA_DIR}/${RES_FILE_NAME}/${RES_FILE_NAME}_allValidPairs_ontarget"
	    exec_cmd $cmd >> ${logfile} 2>&1
	    
            cmd="mv ${DATA_DIR}/${RES_FILE_NAME}/${RES_FILE_NAME}_allValidPairs_ontarget ${DATA_DIR}/${RES_FILE_NAME}/${RES_FILE_NAME}_allValidPairs"
	    exec_cmd $cmd >> ${logfile} 2>&1
	...
	if [[ ! -z ${ALLELE_SPECIFIC_SNP} ]]; then
	    ...
	    cmd="${PYTHON_PATH}/python ${SCRIPTS}/split_valid_interactions.py -i ${DATA_DIR}/${RES_FILE_NAME}/${RES_FILE_NAME}_allValidPairs -v >> ${LDIR}/merge_valid_interactions.log"
	    exec_cmd $cmd >> ${logfile} 2>&1
	fi
    fi
    wait
