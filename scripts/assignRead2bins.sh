...
do
	...
	do
	    ...
	    cat ${DATA_DIR}/${RES_FILE_NAME}/*.validPairs | sed -e 's/\t\t/\t/' | ${SCRIPTS}/build_matrix --public-data --binsize ${bsize} --chrsizes $GENOME_SIZE --ifile /dev/stdin --oprefix ${MATRIX_DIR}/${bsize}/${RES_FILE_NAME}_${bsize} &
	done
    fi
    wait
done
